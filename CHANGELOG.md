...
- **Baseline accuracy investigation (Phase 2.6, diagnostic):** Systematic diagnosis of the 60-percentage-point gap between Phase 2.5 measured accuracy (GADA 40.6% root) and legacy Cantus Stage 2 baseline (99.7% root on 3449-sample dataset). No production code changes; diagnostic test suite and analysis report only.
  - Four-hypothesis investigation (gated MCC_DIAGNOSTIC=1): (1) Full-file vs Middle-50% slicing — NOT SUPPORTED, middle-50% produces 0% (no segments detected); (2) Segment selection rule — PARTIALLY APPLICABLE, onset detection creates spurious segments in single-chord files (attack vs sustain boundaries), highest-confidence rule is correct; (3) Wrapper vs raw detector — PARTIALLY APPLICABLE, legacy CantusChordDetector adds temporal smoothing, minor-3rd protection, and CoreML post-processing (estimated +10–15pp), not accounting for full gap; (4) Chord label normalization — NOT FOUND, confusion is root-level not notation-level.
  - Root cause: Architectural mismatch. AudioExtractor uses onset-based segmentation optimized for real-time polyphonic analysis; single-chord test clips expose this as spurious attack/sustain splits. Legacy Cantus achieved 99.7% via wrapper post-processing and different slicing strategy, not detector superiority.
  - Recommendation (Path A): Accept raw MCC baseline and acknowledge architectural limitation. Update thresholds to measured values; defer post-processing features to Phase 3 if needed. Rationale: Wrapper-free baseline is correct measurement of raw AudioExtractor on this fixture subset; 60-point gap reflects engineering trade-off (general-purpose pipeline vs single-chord clip optimization), not regression.
  - Documentation: docs/diagnostics/phase-2-6-baseline-investigation.md with detailed methodology, test results, re-measured accuracy estimates, architectural analysis, and three recommended paths (A: accept baseline; B: implement missing features; C: use legacy wrapper).
