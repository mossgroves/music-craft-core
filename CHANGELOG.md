...
### Added — AudioExtractor note-source switch (toward 0.0.15)
- **`AudioExtractor.Configuration.noteSource: NoteSource` (`.dsp` default / `.basicPitch`).** `extract` can now produce its `Result` from the Basic Pitch + note-native chord path instead of the YIN + FFT-chroma DSP path. **Default is `.dsp`, so behavior is unchanged until explicitly flipped** — the `.dsp` branch is the original pipeline verbatim, the `Result` shape is preserved, and the Sanctuary call site needs no change (`Configuration()` / `.default` stay byte-identical; `noteSource` is the last init param with a default). Under `.basicPitch`: one `BasicPitchTranscriber` pass (model compiled+loaded once, process-wide, via a cached `static let` — no per-call recompile) feeds `noteNativeChordSegments` (full-polyphony → `NoteChordIdentifier`, 1.0 s/0.5 s windows collapsed into contiguous non-overlapping segments) for chords, a `skyline` melodic reduction for `detectedNotes`, and the **reused** `deriveContour` / `inferKey` / `Result` machinery for the rest (the accurate note-native chords carry the full-poly signal into the chord-based key branch). `ChordSegment.DetectionMethod` is **not** extended — segments reuse the existing `.classifier` case (the enum is frozen for Sanctuary; a `.noteNative` case is a later additive option). Yellow: touches core `AudioExtractor` but additive and default-off. The `.basicPitch` path loads a Core ML model, so the type's "pure / no-I/O" property now holds only for `.dsp`.
