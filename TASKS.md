...
2. **AudioExtractor silence gate (0.1.0 follow-up)** — The Basic Pitch path (the only path as of 0.1.0) does not gate pure-digital-silence input: an all-zero buffer yields a few spurious low-confidence notes and therefore a key (`AudioExtractorTests.testSilentBufferIsWellFormed` documents the behavior; measured 16 notes / 2 segments / a key on a 2 s all-zero buffer). The removed DSP path gated this via an energy/onset threshold. Real captures carry a noise floor, so impact is likely limited to very quiet takes, but Sanctuary could show a spurious chord/key on a near-silent capture. Candidate fix: a cheap peak/RMS-energy guard at the head of `extract` that returns an empty `Result` below a floor. Needs device validation (the floor must not suppress genuine quiet nylon fingerpicking) — that is why it was not added in the 0.1.0 consolidation.
