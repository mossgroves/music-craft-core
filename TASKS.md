...
0. **GuitarSet Phase 3 benchmarks fail against never-calibrated thresholds (found 2026-08-13, the 0.1.14 release round).** `GuitarSetKeyInferenceTests` (thresholds 60% exact / 75% relative, set at creation f210f38 2026-04-28 with "will adjust after first-run measurement" — the adjustment never happened) and `GuitarSetProgressionTests` (mean CSR ≥ 50%) fail on any machine carrying the downloaded 5-fixture GuitarSet subset; they `XCTSkip` everywhere else, which is why release rounds never saw them until the dataset landed locally. Measured 2026-08-13: key exact 20–40% ACROSS RUNS AT THE SAME COMMIT (run-to-run nondeterminism in the key path — investigate before trusting any threshold), CSR 31.1% at 0.1.13 vs 32.2% at 0.1.14 — **confirmed pre-existing by a detached-checkout run at 0.1.13; NOT a 0.1.14 regression** (0.1.14 nudges CSR up). The fixtures are professional funk/jazz comping, far from the nylon fingerpicking MCC is tuned for, and the tests' own calibration-down rule says a >15pp gap from the literature baseline is a finding to surface, not a threshold to lower. When prioritized: (a) chase the nondeterminism, (b) then either calibrate thresholds against a measured baseline or gate the bench behind an env flag (e.g. `MCC_RUN_GUITARSET_BENCH=1`) so the default suite is honest-green while the bench stays runnable. Full logs: Sanctuary session scratchpad 2026-08-13.

1. **LyricsExtractor hypothesis flattening bug** — Lines 74–83 of Sources/MusicCraftCore/Voice/LyricsExtractor.swift flatten all hypotheses from `result.transcriptions` instead of using `.first?.segments`. Causes token duplication in Sanctuary lyricDraft ("I am here I am here I am here" instead of "I am here"). SFSpeechRecognitionResult.transcriptions are ordered by likelihood (best first); MCC should use only the first transcription. Impact: Sanctuary workaround in FragmentAnalyzer.firstHypothesisOnly() detects hypothesis boundaries by finding first token repeat. Fix: replace line 74 `result.transcriptions.flatMap { transcription in` with `(result.transcriptions.first?.segments ?? []).map {` or document why all hypotheses are intentionally concatenated.
